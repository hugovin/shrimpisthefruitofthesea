...
var MooFlow = new Class({
	...
	createMooFlowElement: function(counter, i){
		...
		img.setStyles({'vertical-align':'bottom', 'width':'95%', 'height':'95%'});
	...
	createUI: function(){
		...
		this.nav = new Element('div').addClass('mfNav').setStyle('bottom','-100px');
	...
	setScreen: function(){
		if(this.isFull = !this.isFull){
			...
			this.MooFlow.setStyles({'position':'absolute','z-index':'100','top':'0','left':'0','width':window.getSize().x,'':window.getSize().y});
			...
			this.MooFlow.setStyles({'position':'relative','z-index':'','top':'0','left':'','width':'','height':this.MooFlow.retrieve('height')});
